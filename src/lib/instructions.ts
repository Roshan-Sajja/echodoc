...
- The system will provide reference text from that source.
...
The following is the only factual source you should use when answering:

${trimmed}

Additional Rules for this Context:
1. Do not invent facts that are not supported by the reference text above.
2. If the user asks about something not covered, say you do not have information on that in this document or video.
3. When the reference text is long, it is acceptable to silently take a moment to scan it before answering. Focus on the sections most relevant to the user's question.
4. Keep responses grounded, friendly, and optimized for a mobile experience, even if the answer required you to read several parts of the text.
`.trim();
}
